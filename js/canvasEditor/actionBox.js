...
/*绑定鼠标点击*/
		...
		if (canvas.getActiveObject()) {
			...
			for (var i=0;i<canvas_name_array.length;i++){
				if ($("#"+canvas_name_array[i]).parent().is(":visible")){
					...
					var angle = canvas_array[i].getActiveObject().angle.toFixed(1);
		...
		if (canvas.getActiveObject()) {
			...
			if (object.text == undefined) {
				if(canvas.getActiveObject().stroke == null){
					...
					//$(".pic").click();
					youce("pic");
					$("#picWidth").val(Math.round(object.scaleX*object.width));
					$("#picHeight").val(Math.round(object.scaleY*object.height));
					$("#picRotate").val(angle);
				}else{
					//图形
					youce("print");
					//$(".print").click();
					$("#print_width").val(Math.round(object.scaleX*object.width));
					$("#print_height").val(Math.round(object.scaleY*object.height));
					$("#rotate_ang").val(angle);
				}
			}else{
				youce("font");
			}
	...
	function youce(value) {
		$(".base_div").hide();
		$(".font_div").hide();
		$(".pic_div").hide();
		$(".print_div").hide();
		$("."+value+"_div").show();
			$("."+value).find("img").attr("src", "../images/mipmap-xhdpi/down.png");
			$(".r_t_c").css({
				"color": "black",
